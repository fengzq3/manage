...
//设置全局dialog,依赖customDialog.js
    ...
    function setOfferEvent(status, data) {
        ...
        orderOfferList.find('a').off('click').on('click', function (e) {
            ...
            if (!$(this).hasClass('on')) {
                ...
                offerPic.width('30%');
                //显示detail面板，占位数据
                offerDetail.width('40%').html('<div class=\"noDate\"><div class=\"noData-inner\"><img src=\"/xhltpl/default/img/listload.gif\" /><p>数据载入中...</p></div></div>');
                //获取数据
                $.ajax({type: 'get', url: thisTargetUrl})
                    .then(setOfferDetail, function (e) {
                        //获取失败，处理
                        offerDetail.html('<div class=\"noDate\"><img src=\"/xhltpl/default/img/error.png\" /><p>获取数据失败，请重试！</p></div>');
                    });
            } else {
        ...
        function setOfferDetail(data) {
            ...
            offerPanelConfirm.on('click', function (e) {
                ...
                offerDetail.append('<div class=\"noDate\"><div class=\"noData-inner\"><img src=\"/xhltpl/default/img/listload.gif\" /><p>提交中...</p></div></div>');
                //处理url
                $.ajax({type: 'get', url: thisUrl})
                    .then(function (data) {
                        ...
                        if (data.error === 0) {
                            //返回成功
                            load.showLoad(data.message, 'load-success', 2500);
                            offerDetail.find('.noDate').html('<div class=\"noData-inner\"><img src=\"/xhltpl/default/img/success.png\" /><p>' + data.message + '</p></div>');
                            ...
                            //返回失败
                            load.showLoad(data.message, 'load-warning', 2500);
                            offerDetail.find('.noDate').html('<div class=\"noData-inner\"><img src=\"/xhltpl/default/img/error.png\" /><p>' + data.message + '</p></div>');
                            if (typeof timer !== 'undefined') {
                                clearTimeout(timer);
                            }
                            timer = setTimeout(function () {
    ...
    function chooseWorkFinal() {
        ...
        $.ajax({type: 'get', url: chooseWorkFinalUrl})
            .then(function (data) {
                ...
                designerWorksContent.html('<div class=\"noDate\"><img src=\"/xhltpl/default/img/error.png\" /><p>拉取数据失败，刷新重试！</p></div>');
            });
        //设计稿区 END
        //签单流程区开始
    ...
    function getOrderFlow() {
        ...
        orderFlow.html('<div class=\"noDate\"><img src=\"/xhltpl/default/img/listload.gif\" /><p>载入中...</p></div>');
        $.ajax({type: 'get', url: orderFlowUrl})
            .then(function (data) {
                ...
                orderFlow.addClass('now').html('<div class=\"noDate\"><img src=\"/xhltpl/default/img/error.png\" /><p>拉取开工信息失败，刷新重试！</p></div>');
            });
    }
    ...
    if (chooseOem.length !== 0) {
        chooseOem.on('click', function (e) {
            ...
            //显示loading状态
            chooseOemBtn.html('<div class=\"noDate\"><div class=\"noData-inner\"><img src=\"/xhltpl/default/img/listload.gif\" /><h2>loading...</h2></div></div>');
